#Plotting stream solute concentrations for "temporal carbon" analysis
#CTS 2026-03-24
...
#---- Read stream concentration data ----

#This code taken from EDI

# Package ID: knb-lter-hbr.208.14 Cataloging System:https://pasta.edirepository.org.
# Data set title: Continuous precipitation and stream chemistry data, Hubbard Brook Ecosystem Study, 1963 – ongoing..
...
# Contact:  Christopher Solomon -  Cary Institute of Ecosystem Studies  - solomonc@caryinstitute.org
...
#Change pHmetrohm from factor to numeric
dt2$pHmetrohm <- as.numeric(as.character(dt2$pHmetrohm))
...
#---- Calculate mean annual concentrations  ----

#These are boneheaded-simple calculations of mean annual, not thinking about
#differnces in data density or anything like that.

#Error in this code right now
annualSum <- d %>%
  select(site,date,timeEST,pH,DIC,spCond,temp,ANC960,ANCMet,waterYr,Ca,Mg,K,Na,TMAl,OMAl,Al_ICP,Al_ferron,NH4,SO4,NO3,Cl,PO4,DOC,TDN,DON,SiO2,Mn,Fe,pHmetrohm) %>%
  filter(site%in%c('W1','W2','W3','W4','W5','W6','W7','W8','W9')) %>%
  group_by(year=year(date),site) %>%
  summarize(across(c(pH,DIC,spCond,temp,ANC960,ANCMet,Ca,Mg,K,Na,TMAl,OMAl,Al_ICP,Al_ferron,NH4,SO4,NO3,Cl,PO4,DOC,TDN,DON,SiO2,Mn,Fe,pHmetrohm)),
            ~mean(.x,na.rm=TRUE))
